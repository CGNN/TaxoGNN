...
class TaxonDataset(object):
    ...
    def _query_children(self, query_node):
        return list(nx.bfs_tree(self.taxonomy, self.tx_id2taxon[query_node], depth_limit=1).nodes())
    def _load_dataset_raw(self, dir_path): # dir_path = /dataset/MAG
        ...
        node_file_name = dir_path + '/id2label.txt'
        edge_file_name = dir_path + '/taxonomy.txt'
        ...
        with open(edge_file_name, "r") as fin:
            for line in fin:
                taxo_id_list = line.strip().split()
                parent_taxon = tx_id2taxon[taxo_id_list[0]]
                for i in taxo_id_list[1:]:
                    child_taxon = tx_id2taxon[i]
                    taxonomy.add_edge(parent_taxon, child_taxon)
                    self.parent_map[i] = taxo_id_list[0]
        ...
        # get root node 
        root_node = [n for n,d in taxonomy.in_degree() if d==0][0]
        #list(nx.bfs_tree(taxonomy, root_node, depth_limit=1).nodes())
        ...
        '''
        self.g_full.add_nodes(len(node_id2tx_id))
    ...
    def init_depth(self):
        orig_roots = [v for v, d in self.taxonomy.in_degree() if d == 0]
        if len(orig_roots) != 1: # only one root
            print([k.display_name for k in orig_roots])
            print(len(orig_roots))
            # exit(1)
        # orig_root = orig_roots[0]
        # self.depth_dict = nx.shortest_path_length(self.taxonomy, orig_root)
        list_of_dict = []
        key_set = set()
        self.depth_dict = {}
        for i, orig_root in enumerate(orig_roots):
            temp_dict = nx.shortest_path_length(self.taxonomy, orig_root)
        ...
        for key in key_list:
            min_dist = 100000
            for temp_depth_dict in list_of_dict:
                if key not in temp_depth_dict:
                    continue
                else:
                    min_dist = temp_depth_dict[key] if temp_depth_dict[key] < min_dist else min_dist
                    assert min_dist >= 0
            self.depth_dict[key] = min_dist

    def get_depth(self, tx_id):
        node = self.tx_id2taxon[str(tx_id)]
